...
const ART = [
  ...
  g => { for (let i=0;i<4;i++) g.appendChild(el('line',{x1:45,y1:40+i*55,x2:370,y2:40+i*55,stroke:'#d6d2c8'}));
    ...
    g.appendChild(el('path',{d:'M55 240 L360 95',stroke:'#5a5852','stroke-width':1.6,fill:'none'})); },
  g => { for (let i=0;i<8;i++){
      g.appendChild(el('rect',{x:38,y:34+i*29,width:108,height:20,rx:2,fill:'#d9d5cb'}));
      g.appendChild(el('rect',{x:254,y:34+i*29,width:108,height:20,rx:2,fill:'#d9d5cb'}));
      const t=34+((i*3)%8)*29;
      g.appendChild(el('path',{d:`M146 ${44+i*29} C 200 ${44+i*29}, 200 ${t+10}, 254 ${t+10}`,stroke:'#d99a2b','stroke-opacity':.5,fill:'none'})); } },
  g => { g.appendChild(el('rect',{x:40,y:28,width:140,height:244,rx:3,fill:'#dcd8ce'}));
    for (let i=0;i<17;i++) g.appendChild(el('rect',{x:52,y:42+i*13,width:40+rnd()*76,height:4,rx:2,fill:'#b8b3a7'}));
  ...
  g => { for (let i=0;i<7;i++) g.appendChild(el('line',{x1:30,y1:40+i*36,x2:370,y2:40+i*36,stroke:'#d6d2c8'}));
